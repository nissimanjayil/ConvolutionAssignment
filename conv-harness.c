...
void team_conv_sparse(float *** image, struct sparse_matrix *** kernels,
		       float *** output, int width, int height,
		       int nchannels, int nkernels, int kernel_order) {
  int c,m,h,w,x,y,k,non_zero_index;
  //int product = nkernels * height * width * nchannels;
  float value;


  float *tmp = malloc(sizeof(float) * nchannels * height * width);
#pragma omp parallel for private(c,m,h,w,x,y,k,non_zero_index) shared(nkernels,height,width, nchannels)
